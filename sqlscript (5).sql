...
FROM customer 
WHERE EXISTS ( 
  SELECT 1 FROM loan WHERE customer.customer_id = loan.customer_id 
) OR EXISTS ( 
  SELECT 1 FROM account WHERE customer.customer_id = account.customer_id 
);
